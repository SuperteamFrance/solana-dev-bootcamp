...
import { getCounterdappProgram, getCounterdappProgramId } from '@project/anchor'
...
export function useCounterdappProgram() {
  const { connection } = useConnection()
  const { cluster } = useCluster()
  const transactionToast = useTransactionToast()
  const provider = useAnchorProvider()
  const programId = useMemo(() => getCounterdappProgramId(cluster.network as Cluster), [cluster])
  const program = useMemo(() => getCounterdappProgram(provider, programId), [provider, programId])
...
export function useCounterdappProgramAccount({ account }: { account: PublicKey }) {
  const { cluster } = useCluster()
  const transactionToast = useTransactionToast()
  const { program, accounts } = useCounterdappProgram()
